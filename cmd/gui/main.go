...
func main() {
	...
	selectTemplateCheckDirBtn := widget.NewButton("Select templates folder for checking", func() {
		dialog.ShowFolderOpen(func(uri fyne.ListableURI, err error) {
			...
			templatesDir = uri.Path() 
			templateCheckLabel.SetText("Template folder: " + templatesDir)
		}, w)
	})
	...
	createTemplateBtn := widget.NewButton("Create new template", func() {
		...
		if url == "" {
			dialog.ShowInformation("Error", "Please enter a URL first", w)
			return
		}

		yamlContent := templates.GenerateTemplate(url) 
		...
		saveDialog := dialog.NewFileSave(func(uc fyne.URIWriteCloser, err error) {
			...
			_, err = uc.Write([]byte(yamlContent))
			if err != nil {
				dialog.ShowError(err, w)
				return
			}
			dialog.ShowInformation("Success", "Template saved successfully", w)
		}, w)
	...
	checkTemplatesBtn := widget.NewButton("Check templates", func() {
		if templatesDir == "" {
			dialog.ShowInformation("Error", "Please select a templates folder", w)
			return
		}
		url := strings.TrimSpace(urlEntry.Text)
		if url == "" {
			dialog.ShowInformation("Error", "Please enter a URL", w)
			return
		}

		matched, err := templates.FindMatchingTemplates(url, templatesDir)
		if err != nil {
			dialog.ShowError(err, w)
			return
		}
	...
	createTemplateBtn = widget.NewButton("Create new template", func() {
		...
		content := templates.GenerateTemplate(url)
		...
		saveDialog := dialog.NewFileSave(func(uc fyne.URIWriteCloser, err error) {
			...
			defer uc.Close()
			_, err = uc.Write([]byte(content))
			if err != nil {
				dialog.ShowError(err, w)
				return
			}
			dialog.ShowInformation("Done", "Template saved successfully", w)
		}, w)
	...
	scannerSection := container.NewVBox(
	...
	templateCheckSection := container.NewVBox(
		widget.NewLabel("Template Checking & Generation"),
		urlEntry,
